...
通常読みの響き、数字の見た目、語呂合わせを別々に順位付けする Python プロジェクトです。
...
| `run/TOP.md` | 音と読みやすさ、見た目・数字構造の TOP |
...
- **TOP 30 — 見た目・数字構造**: 反復、回文、鏡映、連番などの明確な数値パターン
- **TOP 30 — 語呂合わせ**: [masks.txt](masks.txt) の読みを使った日本語の語呂合わせ
- **TOP 10 — 新しく見つかった番号**: 同じ scope の前回 snapshot に存在しなかった
  番号を通常読みで順位付けし、候補が10件以上ある時だけ Release 本文に掲載

完全スキャンは3つのメイン TOP をそれぞれ30件選びます。特定マスクランでは、
候補が少なければその件数までです。

Python が音と数字構造の候補を別々の再現可能な特徴量で抽出し、同じマスク系統への
偏りを抑えます。Codex は候補 ID だけを順位付けします。Python が現在の snapshot に
対して ID、件数、読み、系統の上限を検証して Markdown を生成し、不正な選択は
最大3回まで再試行します。
